...
TEST(Component, InstantiationPropagatesResourceExport) {
  // Regression test for GAP-I-3: A component that defines and exports a
  // resource type must have its ResourceId tracked during raw-Component
  // instantiation, so that a subsequent alias-export can resolve the resource.
  //
  // (component
  //   (component $C
  ...
  //   (alias export $c "r" (type $R_alias))
  //   (type $R_borrow (borrow $R_alias))
  // )
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  ...
  std::vector<uint8_t> Vec = {
      ...
      0x52, 0x05, 0x04, 0x01, 0x00, 0x00, 0x00, 0x06, 0x06, 0x01, 0x03, 0x00,
      0x00, 0x01, 0x72, 0x07, 0x03, 0x01, 0x68, 0x00, 0x00, 0x34, 0x0e, 0x63,
      0x6f, 0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e, 0x74, 0x2d, 0x6e, 0x61, 0x6d,
      0x65, 0x01, 0x15, 0x03, 0x02, 0x00, 0x07, 0x52, 0x5f, 0x61, 0x6c, 0x69,
      0x61, 0x73, 0x01, 0x08, 0x52, 0x5f, 0x62, 0x6f, 0x72, 0x72, 0x6f, 0x77,
